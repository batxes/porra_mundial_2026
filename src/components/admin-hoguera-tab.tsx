...
type HogueraAdminStatus = {
  active?: boolean;
  active_hoguera_id?: string | null;
  active_hoguera_title?: string | null;
  total_attempts?: number;
  updated_at?: string | null;
};

type HogueraAttemptRow = {
  awarded_drop_ids?: string[];
  completed_at?: string | null;
  display_name?: string | null;
  hoguera_id?: string;
  hoguera_title?: string | null;
...
export function AdminHogueraTab() {
  const { usingSupabase, user } = useAppContext();
  const [status, setStatus] = useState<HogueraAdminStatus | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [statsOpen, setStatsOpen] = useState(false);
  const [attempts, setAttempts] = useState<HogueraAttemptRow[]>([]);
  ...
  const loadStatus = useCallback(async () => {
    ...
    const { data, error: statusError } = await supabase.rpc(
      "admin_hoguera_status",
    );
    if (statusError) {
      setError(
        isMissingRpcError(statusError)
          ? "Falta aplicar la migracion de la hoguera en esta base."
          : statusError.message,
      );
      return;
    }
    setError("");
    setStatus(firstRow<HogueraAdminStatus>(data));
  ...
  const setActive = async (active: boolean) => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc(
        "admin_set_hoguera_active",
        { p_active: active },
      );
      if (rpcError) throw new Error(rpcError.message);
      setStatus(firstRow<HogueraAdminStatus>(data));
      toast.success(active ? "Hoguera activada" : "Hoguera pausada");
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
          : "No se ha podido actualizar la hoguera.";
      setError(msg);
      toast.error("No se ha podido actualizar la hoguera", {
        description: msg,
      });
    } finally {
      setBusy(false);
  ...
  const loadAttempts = useCallback(async () => {
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc(
        "admin_hoguera_attempts",
        { p_hoguera_id: null },
      );
      if (rpcError) throw new Error(rpcError.message);
      setAttempts(rows<HogueraAttemptRow>(data));
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-xl font-semibold text-white">Hoguera</h3>
        <p className="mt-1 text-sm text-zinc-400">
          Minijuego Salta la hoguera: cada jugador hace un intento (3 vidas, mejor
          marca) y el servidor reparte los sobres privados segun los metros que
          alcance.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <PrizeCard title="Sobre Defensas" subtitle="25 m" />
        <PrizeCard title="Sobre Mediocentros" subtitle="50 m" />
        <PrizeCard title="Sobre Premier" subtitle="75 m" />
        <PrizeCard title="Sobre Promesas" subtitle="100 m" />
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-zinc-500">
              ...
              {status?.active_hoguera_title || "SALTA LA HOGUERA"}
            </p>
...
function HogueraStatsModal({
  attempts,
  loading,
  onClose,
  onRefresh,
}: {
  attempts: HogueraAttemptRow[];
  loading: boolean;
  onClose: () => void;
  onRefresh: () => void;
}) {
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="hoguera-stats-title"
    >
            ...
            <table className="w-full min-w-[620px] text-left text-sm">
              <thead className="sticky top-0 bg-[#171717] text-[11px] uppercase tracking-[0.12em] text-zinc-400">
                <tr>
                  ...
                  <th className="px-3 py-2 font-bold">Distancia</th>
                  <th className="px-3 py-2 font-bold">Meta</th>
                ...
                {attempts.map((attempt) => (
                  <tr key={`${attempt.user_id}-${attempt.completed_at}`}>
                    ...
                    <td className="px-3 py-2 font-bold text-[#a7f600]">
                      {Number(attempt.meters_reached || 0)} m
                    </td>
                    <td className="px-3 py-2 text-zinc-300">
                      {attempt.reached_goal ? "Si" : "-"}
                    </td>
                    <td className="px-3 py-2 text-zinc-300">
