...
type SuarezDentistAdminStatus = {
  active?: boolean;
  active_suarez_dentist_id?: string | null;
  active_suarez_dentist_title?: string | null;
  total_attempts?: number;
  updated_at?: string | null;
};

type SuarezDentistAttemptRow = {
  attempts?: number[];
  ...
  suarez_dentist_title?: string | null;
  user_id?: string;
};
...
export function AdminSuarezDentistTab() {
  ...
  const [status, setStatus] = useState<SuarezDentistAdminStatus | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [statsOpen, setStatsOpen] = useState(false);
  const [attempts, setAttempts] = useState<SuarezDentistAttemptRow[]>([]);
  ...
  const loadStatus = useCallback(async () => {
    ...
    const { data, error: statusError } = await supabase.rpc(
      "admin_suarez_dentist_status",
    );
    if (statusError) {
      setError(
        isMissingRpcError(statusError)
          ? "Falta aplicar la migracion de Suarez en esta base."
          : statusError.message,
      );
      return;
    }
    setError("");
    setStatus(firstRow<SuarezDentistAdminStatus>(data));
  ...
  const setActive = async (active: boolean) => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc(
        "admin_set_suarez_dentist_active",
        { p_active: active },
      );
      if (rpcError) throw new Error(rpcError.message);
      setStatus(firstRow<SuarezDentistAdminStatus>(data));
      toast.success(active ? "Suarez activado" : "Suarez pausado");
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
          : "No se ha podido actualizar Suarez.";
      setError(msg);
      toast.error("No se ha podido actualizar Suarez", {
        description: msg,
      });
    } finally {
      setBusy(false);
  ...
  const loadAttempts = useCallback(async () => {
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc(
        "admin_suarez_dentist_attempts",
        { p_suarez_dentist_id: null },
      );
      if (rpcError) throw new Error(rpcError.message);
      setAttempts(rows<SuarezDentistAttemptRow>(data));
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-xl font-semibold text-white">Dentista Suarez</h3>
        <p className="mt-1 text-sm text-zinc-400">
          Minijuego Crocodile Dentist: cada jugador tiene 2 vidas, cuenta su
          mejor partida y el servidor reparte 1 sobre extra por cada diente
          salvado.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        <PrizeCard title="Sobre Defensas" subtitle="Diente 1" />
        <PrizeCard title="Sobre Mediocentros" subtitle="Diente 2" />
        <PrizeCard title="Sobre Premier" subtitle="Diente 3" />
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-zinc-500">
              ...
              {status?.active_suarez_dentist_title || "DENTISTA SUAREZ"}
            </p>
...
function SuarezDentistStatsModal({
  attempts,
  loading,
  onClose,
  onRefresh,
}: {
  attempts: SuarezDentistAttemptRow[];
  loading: boolean;
  onClose: () => void;
  onRefresh: () => void;
}) {
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="suarez-dentist-stats-title"
    >
