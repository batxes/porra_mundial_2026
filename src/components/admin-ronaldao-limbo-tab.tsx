...
type RonaldaoLimboAdminStatus = {
  active?: boolean;
  active_ronaldao_limbo_id?: string | null;
  active_ronaldao_limbo_title?: string | null;
  total_attempts?: number;
  updated_at?: string | null;
};

type RonaldaoLimboAttemptRow = {
  ...
  ronaldao_limbo_title?: string | null;
...
function formatRoundScores(value?: unknown) {
  if (!Array.isArray(value) || !value.length) return "-";
  return value
    .map((score, index) => `V${index + 1}: ${Number(score) || 0}`)
    .join(" / ");
}

export function AdminRonaldaoLimboTab() {
  const { usingSupabase, user } = useAppContext();
  const [status, setStatus] = useState<RonaldaoLimboAdminStatus | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [statsOpen, setStatsOpen] = useState(false);
  const [attempts, setAttempts] = useState<RonaldaoLimboAttemptRow[]>([]);
  ...
  const loadStatus = useCallback(async () => {
    ...
    const { data, error: statusError } = await supabase.rpc(
      "admin_ronaldao_limbo_status",
    );
    if (statusError) {
      setError(
        isMissingRpcError(statusError)
          ? "Falta aplicar la migracion de Ronaldao en esta base."
          : statusError.message,
      );
      return;
    }
    setError("");
    setStatus(firstRow<RonaldaoLimboAdminStatus>(data));
  ...
  const setActive = async (active: boolean) => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc(
        "admin_set_ronaldao_limbo_active",
        { p_active: active },
      );
      if (rpcError) throw new Error(rpcError.message);
      setStatus(firstRow<RonaldaoLimboAdminStatus>(data));
      toast.success(active ? "Ronaldao activado" : "Ronaldao pausado");
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
          : "No se ha podido actualizar Ronaldao.";
      setError(msg);
      toast.error("No se ha podido actualizar Ronaldao", {
        description: msg,
      });
    } finally {
      setBusy(false);
  ...
  const loadAttempts = useCallback(async () => {
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc(
        "admin_ronaldao_limbo_attempts",
        { p_ronaldao_limbo_id: null },
      );
      if (rpcError) throw new Error(rpcError.message);
      setAttempts(rows<RonaldaoLimboAttemptRow>(data));
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-xl font-semibold text-white">Patata caliente</h3>
        <p className="mt-1 text-sm text-zinc-400">
          Minijuego Ronaldao al limbo: cada jugador tiene 3 vidas, cuenta su
          mejor intento y el servidor reparte hasta 6 sobres privados.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        <PrizeCard title="Sobre Defensas" subtitle="Paso 1" />
        <PrizeCard title="Sobre Porteros" subtitle="Paso 2" />
        <PrizeCard title="Sobre Delanteros" subtitle="Paso 3" />
        <PrizeCard title="Sobre Mediocentros" subtitle="Paso 4" />
        <PrizeCard title="Sobre Promesas" subtitle="Paso 5" />
        <PrizeCard title="Sobre Estrellas" subtitle="Paso 6" />
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-zinc-500">
              ...
              {status?.active_ronaldao_limbo_title || "PATATA CALIENTE"}
            </p>
...
function RonaldaoLimboStatsModal({
  attempts,
  loading,
  onClose,
  onRefresh,
}: {
  attempts: RonaldaoLimboAttemptRow[];
  loading: boolean;
  onClose: () => void;
  onRefresh: () => void;
}) {
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ronaldao-limbo-stats-title"
    >
                ...
                {attempts.map((attempt) => (
                  <tr key={`${attempt.user_id}-${attempt.completed_at}`}>
                    ...
                    <td className="px-3 py-2 font-bold text-[#f5c518]">
                      {Number(attempt.packs_awarded || 0)}/6
                    </td>
                    <td className="px-3 py-2 text-zinc-300">
                      {formatRoundScores(attempt.round_scores)}
                    </td>
                    <td className="px-3 py-2 text-zinc-300">
