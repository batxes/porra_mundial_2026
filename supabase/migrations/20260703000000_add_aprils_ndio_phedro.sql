-- Segunda carta del sobre Leyendas: Ndio Yankler.
-- Mantiene las cartas Aprils fuera de pools normales y de la forja.
...
where player_id in ('apr-bicho', 'apr-indio');
...
as $$
  select case
    ...
    when p_player_id = 'apr-indio' then 6
    else (
      select coalesce(sum(
        case e.event_type
          when 'goal' then
...
begin
  ...
  if exists (
    ...
    where c.id = any(v_ids)
      and c.user_id = v_uid
      and c.player_id in ('apr-bicho', 'apr-indio')
  ) then
